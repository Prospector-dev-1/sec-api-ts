...
__export(index_exports, {
  ...
  aaerApi: () => aaerApi,
  auditFeesApi: () => auditFeesApi,
  datasetsApi: () => datasetsApi,
  default: () => index_default,
  directorsBoardMembersApi: () => directorsBoardMembersApi,
  downloadApi: () => downloadApi,
  edgarEntitiesApi: () => edgarEntitiesApi,
  edgarIndexApi: () => edgarIndexApi,
  execCompApi: () => execCompApi,
  extractorApi: () => extractorApi,
  floatApi: () => floatApi,
  form13DGApi: () => form13DGApi,
  form13FCoverPagesApi: () => form13FCoverPagesApi,
  form13FHoldingsApi: () => form13FHoldingsApi,
  form144Api: () => form144Api,
  form1AApi: () => form1AApi,
  form1KApi: () => form1KApi,
  form1ZApi: () => form1ZApi,
  form8KApi: () => form8KApi,
  formAdvApi: () => formAdvApi,
  formCApi: () => formCApi,
  formDApi: () => formDApi,
  formNcenApi: () => formNcenApi,
  formNportApi: () => formNportApi,
  formNpxApi: () => formNpxApi,
  formS1424B4Api: () => formS1424B4Api,
  ...
  queryApi: () => queryApi,
  regASearchApi: () => regASearchApi,
  renderApi: () => renderApi,
  secAdminProceedingsApi: () => secAdminProceedingsApi,
  secEnforcementActionsApi: () => secEnforcementActionsApi,
  secLitigationsApi: () => secLitigationsApi,
  setApiKey: () => setApiKey,
  sroFilingsApi: () => sroFilingsApi,
...
var decompressStream = (res) => {
  ...
  if (encoding === "gzip") {
    return res.pipe(import_zlib.default.createGunzip());
  }
  if (encoding === "deflate") {
    return res.pipe(import_zlib.default.createInflate());
  }
  if (encoding === "br") {
    return res.pipe(import_zlib.default.createBrotliDecompress());
  }
  return res;
};
var singleRequest = ({
...
var streamToFile = ({ url, destPath, _redirectCount = 0 }) => {
  return new Promise((resolve, reject) => {
    const parsedUrl = new URL(url);
    const req = import_https.default.request(parsedUrl, { method: "GET", agent }, (res) => {
      const status = res.statusCode || 500;
      if (status >= 300 && status < 400 && res.headers.location) {
        res.resume();
        if (_redirectCount >= MAX_REDIRECTS) {
...
var downloadToFile = async ({
  ...
  if (import_fs.default.existsSync(destPath)) {
    if (expectedSize === void 0 || expectedSize === null || import_fs.default.statSync(destPath).size === expectedSize) {
      return destPath;
    }
  }
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    try {
      ...
      import_fs.default.renameSync(tmpPath, destPath);
      return destPath;
    } catch (error) {
...
var DatasetsApi = class {
  ...
  async download(nameOrOptions) {
    ...
    if (strategy === "zip") {
      ...
      const dest = import_path2.default.join(targetDir2, `${name}.zip`);
    ...
    const targetDir = downloadPath || import_path2.default.join(DEFAULT_DOWNLOAD_DIR, name);
    const containers = dataset.containers || [];
    const downloaded = [];
    for (const container of containers) {
      const url = this.appendToken(container.downloadUrl, this.apiKey);
      const dest = import_path2.default.join(targetDir, container.key);
